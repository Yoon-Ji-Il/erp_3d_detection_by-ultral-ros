...
private:
  ...
  std::string projection_model_;      // "pinhole" | "erp"
  double erp_yaw_offset_deg_{0.0};
  bool erp_flip_x_{false};
  bool erp_flip_y_{false};
  int  last_img_w_{0};
  int  last_img_h_{0};
  std::string camera_frame_;     
  bool projectToPixel(const cv::Point3d& P,
                      double& u, double& v,
